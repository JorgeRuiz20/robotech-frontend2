...
function AdminPanel() {
  ...
  useEffect(() => { loadSedes(); loadJueces(); }, []);
  ...
  // ✅ Protegido: crear/editar torneo
  const [handleCreateTorneo, isCreatingTorneo] = useAsyncAction(async (e) => {
    e?.preventDefault();
    const requestData = {
      ...
      juezResponsableId: parseInt(torneoForm.juezResponsableId)
    };

    if (torneoForm.activacionAutomatica && torneoForm.fechaActivacionProgramada) {
      requestData.fechaActivacionProgramada = torneoForm.fechaActivacionProgramada + ':00.000';
  ...
  const handleEditTorneo = (torneo) => {
    ...
    setTorneoForm({
      ...
      juezResponsableId: torneo.juezResponsableId || '',
  ...
  // ✅ Protegido: eliminar torneo
  const [handleDeleteTorneo, isDeletingTorneo] = useAsyncAction(async (id) => {
  ...
  // ✅ Protegido: cambiar estado torneo
  const [handleCambiarEstadoTorneo, isChangingEstado] = useAsyncAction(async (id, nuevoEstado) => {
  ...
  const resetTorneoForm = () => {
    ...
    setTorneoForm({
      nombre: '', descripcion: '', sedeId: '', categoriaId: '',
      juezResponsableId: '', activacionAutomatica: false, fechaActivacionProgramada: ''
    });
  };

  // ✅ Protegido: crear/editar categoría
  const [handleCreateCategoria, isCreatingCategoria] = useAsyncAction(async (e) => {
    e?.preventDefault();
    if (editingCategoria) {
  ...
  // ✅ Protegido: desactivar categoría
  const [handleDesactivarCategoria, isDesactivating] = useAsyncAction(async (id) => {
  ...
  const resetCategoriaForm = () => {
    ...
    setCategoriaForm({ nombre: '', descripcion: '', pesoMaximo: '', reglasEspecificas: '' });
  };

  // ✅ Protegido: crear usuario
  const [handleCreateUser, isCreatingUser] = useAsyncAction(async (e) => {
    e?.preventDefault();
    ...
    if (response.data.success) {
      ...
      setUserForm({ nombre: '', apellido: '', dni: '', email: '', telefono: '', fechaNacimiento: '', roles: [] });
      loadData();
    }
  });
  ...
  return (
    <div className="dashboard-container">
      ...
      <div className="dashboard-tabs">
        {['torneos','sedes','categorias','usuarios','emailReset','deshabilitacion','codigos','reportes'].map(tab => (
          <button
            key={tab}
          ...
          >
            ...
            {tab === 'emailReset' && '📧 Reset Email'}
            {tab === 'deshabilitacion' && '🚫 Deshabilitar Clubs'}
            {tab === 'codigos' && '🔑 Códigos'}
          ...
          <div>
            <div style={{display: 'flex', justifyContent: 'space-between', marginBottom: '2rem'}}>
              ...
              <button
                ...
                onClick={() => { resetTorneoForm(); setShowTorneoForm(!showTorneoForm); }}
            ...
            {showTorneoForm && (
              <div className="form-card" style={{marginBottom: '2rem'}}>
                ...
                <div className="form-group">
                  <label>Nombre *</label>
                  <input type="text" value={torneoForm.nombre}
                    onChange={(e) => setTorneoForm({...torneoForm, nombre: e.target.value})} required />
                </div>
                <div className="form-group">
                  <label>Descripción</label>
                  <textarea value={torneoForm.descripcion}
                    onChange={(e) => setTorneoForm({...torneoForm, descripcion: e.target.value})} rows="3" />
                </div>
                <div className="form-group">
                  <label>Sede *</label>
                  <select value={torneoForm.sedeId}
                    onChange={(e) => setTorneoForm({...torneoForm, sedeId: e.target.value})} required>
                    <option value="">Selecciona una sede</option>
                    {sedes.map(sede => (
                      <option key={sede.id} value={sede.id}>{sede.nombre} - {sede.distrito}</option>
                    ))}
                  </select>
                ...
                <div className="form-group">
                  ...
                  <select value={torneoForm.categoriaId}
                    onChange={(e) => setTorneoForm({...torneoForm, categoriaId: e.target.value})} required>
                ...
                <div className="form-group">
                  ...
                  <select value={torneoForm.juezResponsableId}
                    onChange={(e) => setTorneoForm({...torneoForm, juezResponsableId: e.target.value})} required>
                ...
                <div style={{background: 'rgba(0,200,255,0.05)', padding: '1.5rem', borderRadius: '8px', border: '1px solid rgba(0,200,255,0.2)', marginTop: '1.5rem'}}>
                  <div className="form-group">
                    <label style={{display: 'flex', alignItems: 'center', gap: '0.5rem', cursor: 'pointer'}}>
                      <input type="checkbox" checked={torneoForm.activacionAutomatica || false}
                        onChange={(e) => setTorneoForm({...torneoForm, activacionAutomatica: e.target.checked, fechaActivacionProgramada: e.target.checked ? torneoForm.fechaActivacionProgramada : ''})} />
                  ...
                  {torneoForm.activacionAutomatica && (
                    <div className="form-group" style={{marginTop: '1rem'}}>
                      ...
                      <input type="datetime-local" value={torneoForm.fechaActivacionProgramada}
                ...
                <button
                  ...
                  disabled={isAnyLoading}
                >
                  {isCreatingTorneo ? '⏳ Guardando...' : (editingTorneo ? '💾 Actualizar' : '✅ Crear')} Torneo
                </button>
              </div>
            )}
              ...
              <div className="data-grid">
                {torneos.map(torneo => (
                  <div key={torneo.id} className="card">
                    ...
                    {torneo.sedeNombre && <p><strong>🏟️ Sede:</strong> {torneo.sedeNombre}</p>}
                    <p><strong>📂 Categoría:</strong> {torneo.categoriaNombre}</p>
                    {torneo.juezResponsableNombre && <p><strong>⚖️ Juez:</strong> {torneo.juezResponsableNombre}</p>}

                    {torneo.activacionAutomatica && torneo.fechaActivacionProgramada && torneo.estado === 'PENDIENTE' && (
                      <div style={{marginTop: '1rem', padding: '1rem', background: 'rgba(0,200,255,0.05)', border: '1px solid rgba(0,200,255,0.2)', borderRadius: '8px'}}>
                        ...
                        <p style={{fontSize: '0.85rem', color: 'var(--muted)', marginBottom: '0.8rem'}}>
                          📅 {new Date(torneo.fechaActivacionProgramada).toLocaleString('es-ES', {day: '2-digit', month: 'long', year: 'numeric', hour: '2-digit', minute: '2-digit'})}
                        </p>
                    ...
                    <div style={{display: 'flex', gap: '0.5rem', marginTop: '1rem', flexWrap: 'wrap'}}>
                      <button className="btn-secondary" onClick={() => handleEditTorneo(torneo)} style={{flex: 1}} disabled={isAnyLoading}>
                        ✏️ Editar
                      </button>
                        ...
                        <>
                          {!torneo.activacionAutomatica && (
                            <button className="btn-primary" style={{flex: 1}} disabled={isAnyLoading}
                              ...
                              {isChangingEstado ? '⏳...' : '▶️ Activar'}
                            </button>
                          )}
                          <button className="btn-secondary" style={{flex: 1, background: 'rgba(255,59,59,0.1)', borderColor: '#ff3b3b'}}
                            disabled={isAnyLoading} onClick={() => handleDeleteTorneo(torneo.id)}>
                            {isDeletingTorneo ? '⏳...' : '🗑️ Eliminar'}
                          </button>
                        </>
                      )}
          ...
          <div>
            <div style={{display: 'flex', justifyContent: 'space-between', marginBottom: '2rem'}}>
              ...
              <button className="btn-primary" disabled={isAnyLoading}
                onClick={() => { resetCategoriaForm(); setShowCategoriaForm(!showCategoriaForm); }}>
            ...
            {showCategoriaForm && (
              <div className="form-card" style={{marginBottom: '2rem'}}>
                ...
                <div className="form-group">
                  <label>Nombre *</label>
                  <input type="text" value={categoriaForm.nombre}
                    onChange={(e) => setCategoriaForm({...categoriaForm, nombre: e.target.value})} required />
                </div>
                <div className="form-group">
                  <label>Descripción</label>
                  <textarea value={categoriaForm.descripcion}
                    onChange={(e) => setCategoriaForm({...categoriaForm, descripcion: e.target.value})} rows="3" />
                </div>
                <div className="form-group">
                  <label>Peso Máximo (g) *</label>
                  <input type="number" value={categoriaForm.pesoMaximo}
                    onChange={(e) => setCategoriaForm({...categoriaForm, pesoMaximo: e.target.value})} required />
                </div>
                <div className="form-group">
                  <label>Reglas Específicas</label>
                  <textarea value={categoriaForm.reglasEspecificas}
                    onChange={(e) => setCategoriaForm({...categoriaForm, reglasEspecificas: e.target.value})} rows="4" />
                </div>
                <button onClick={handleCreateCategoria} className="btn-primary" disabled={isAnyLoading}>
                  {isCreatingCategoria ? '⏳ Guardando...' : (editingCategoria ? 'Actualizar' : 'Crear')} Categoría
                </button>
              </div>
            )}
              ...
              <div className="data-grid">
                {categorias.map(cat => (
                  <div key={cat.id} className="card">
                    ...
                    <p><strong>Estado:</strong> <span className={`badge ${cat.activa ? 'badge-aprobado' : 'badge-rechazado'}`}>{cat.activa ? 'ACTIVA' : 'INACTIVA'}</span></p>
                    <div style={{display: 'flex', gap: '0.5rem', marginTop: '1rem'}}>
                      <button className="btn-secondary" style={{flex: 1}} disabled={isAnyLoading}
                        onClick={() => handleEditCategoria(cat)}>✏️ Editar</button>
                      {cat.activa && (
                        <button className="btn-secondary" style={{flex: 1, background: 'rgba(255,59,59,0.1)', borderColor: '#ff3b3b'}}
                          disabled={isAnyLoading} onClick={() => handleDesactivarCategoria(cat.id)}>
                          {isDesactivating ? '⏳...' : '🚫 Desactivar'}
                        </button>
                      )}
                    </div>
                  </div>
          ...
          <div>
            ...
            {showUserForm && (
              <div className="form-card" style={{marginBottom: '2rem'}}>
                ...
                <div style={{display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem'}}>
                  <div className="form-group">
                    <label>Nombre *</label>
                    <input type="text" value={userForm.nombre}
                      onChange={(e) => setUserForm({...userForm, nombre: e.target.value})} required />
                  </div>
                  <div className="form-group">
                    <label>Apellido *</label>
                    <input type="text" value={userForm.apellido}
                      onChange={(e) => setUserForm({...userForm, apellido: e.target.value})} required />
                  </div>
                </div>
                <div style={{display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem'}}>
                  <div className="form-group">
                    <label>DNI *</label>
                    <input type="text" value={userForm.dni}
                      onChange={(e) => setUserForm({...userForm, dni: e.target.value})} required maxLength="8" />
                  </div>
                  <div className="form-group">
                    <label>Email *</label>
                ...
                <div style={{display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem'}}>
                  <div className="form-group">
                    ...
                    <input type="tel" value={userForm.telefono}
                      onChange={(e) => setUserForm({...userForm, telefono: e.target.value})} required />
                  </div>
                  <div className="form-group">
                    <label>Fecha de Nacimiento *</label>
                    <input type="date" value={userForm.fechaNacimiento}
                      onChange={(e) => setUserForm({...userForm, fechaNacimiento: e.target.value})} required />
                  </div>
                </div>
                <div className="form-group">
                  <label>Roles *</label>
                  <div style={{display: 'flex', flexWrap: 'wrap', gap: '1rem', marginTop: '0.5rem'}}>
                    {['ROLE_ADMIN', 'ROLE_CLUB_OWNER', 'ROLE_JUDGE', 'ROLE_COMPETITOR', 'ROLE_USER'].map(role => (
                      <label key={role} style={{display: 'flex', alignItems: 'center', gap: '0.5rem'}}>
                        <input type="checkbox" checked={userForm.roles.includes(role)}
                ...
                <button onClick={handleCreateUser} className="btn-primary" disabled={isAnyLoading}>
            ...
            {loading ? <div className="loading">Cargando...</div> : (
              <div className="table-container">
                <table>
                  <thead>
                    <tr><th>Nombre</th><th>Email</th><th>DNI</th><th>Estado</th><th>Roles</th></tr>
                  </thead>
                  <tbody>
                    {users.map(user => (
                      <tr key={user.id}>
                        ...
                        <td>{user.email}</td>
                        <td>{user.dni}</td>
                        <td><span className={`badge badge-${user.estado.toLowerCase()}`}>{user.estado}</span></td>
          ...
          <div>
            ...
            {loading ? <div className="loading">Cargando...</div> : (
              <div className="table-container">
                <table>
                  <thead>
                    <tr><th>Código</th><th>Club</th><th>Estado</th><th>Generado Por</th><th>Acciones</th></tr>
                  </thead>
                  <tbody>
                    {codigos.map(codigo => (
                      <tr key={codigo.id}>
                        ...
                        <td><span className={`badge ${codigo.usado ? 'badge-rechazado' : 'badge-aprobado'}`}>{codigo.usado ? 'USADO' : 'DISPONIBLE'}</span></td>
                        <td>{codigo.generadoPorNombre}</td>
                        <td>
                          <button className="btn-secondary" onClick={() => copiarCodigo(codigo.codigo)}>📋 Copiar</button>
                        </td>
                      </tr>
                    ))}
                  </tbody>
