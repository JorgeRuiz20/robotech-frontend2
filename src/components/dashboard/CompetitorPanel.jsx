// src/components/dashboard/CompetitorPanel.jsx
...
function CompetitorPanel() {
  ...
  const [robotForm, setRobotForm] = useState({
    nombre: '', descripcion: '', peso: '', especificacionesTecnicas: '', categoriaId: '', fotoRobot: ''
  });
  ...
  const [transferenciaForm, setTransferenciaForm] = useState({ clubDestinoId: '', mensaje: '' });
  ...
  // ✅ Protegido: crear/editar robot
  const [handleSubmitRobot, isSavingRobot] = useAsyncAction(async () => {
  ...
  const handleEditRobot = (robot) => {
    ...
    setRobotForm({
      nombre: robot.nombre, descripcion: robot.descripcion, peso: robot.peso,
      especificacionesTecnicas: robot.especificacionesTecnicas,
      categoriaId: robot.categoriaId, fotoRobot: robot.fotoRobot || ''
    });
    setShowRobotForm(true);
  };

  // ✅ Protegido: eliminar robot
  const [handleDeleteRobot, isDeletingRobot] = useAsyncAction(async (id) => {
  ...
  const resetRobotForm = () => {
    ...
    setRobotForm({ nombre: '', descripcion: '', peso: '', especificacionesTecnicas: '', categoriaId: '', fotoRobot: '' });
  };

  const handleAbrirInscripcion = (torneo) => {
    const robotsAprobados = misRobots.filter(r => r.estado === 'APROBADO' && r.categoriaId === torneo.categoriaId);
  ...
  // ✅ Protegido: inscribirse a torneo
  const [handleInscribirse, isInscribiendo] = useAsyncAction(async (robotId) => {
  ...
  // ✅ Protegido: cancelar transferencia
  const [handleCancelarTransferencia, isCancelingTransfer] = useAsyncAction(async (solicitudId) => {
  ...
  // ✅ Protegido: enviar solicitud de transferencia
  const [handleEnviarSolicitud, isEnviandoSolicitud] = useAsyncAction(async () => {
    if (!transferenciaForm.clubDestinoId) {
      alert('Selecciona un club destino');
      return;
    }
    await transferenciaService.solicitar(transferenciaForm.clubDestinoId, transferenciaForm.mensaje);
  ...
  return (
    <div className="dashboard-container">
      ...
      <div className="dashboard-tabs">
        <button className={activeTab === 'robots' ? 'active' : ''} onClick={() => setActiveTab('robots')}>🤖 Mis Robots</button>
        <button className={activeTab === 'torneos' ? 'active' : ''} onClick={() => setActiveTab('torneos')}>🏆 Torneos Disponibles</button>
        <button className={activeTab === 'transferencias' ? 'active' : ''} onClick={() => setActiveTab('transferencias')}>🔄 Transferencias</button>
      </div>
      ...
      <div className="dashboard-content">
        ...
        {activeTab === 'robots' && (
          <div>
            <div style={{display: 'flex', justifyContent: 'space-between', marginBottom: '2rem'}}>
              ...
              <button className="btn-primary" disabled={isAnyLoading || misRobots.length >= 5}
                onClick={() => { resetRobotForm(); setShowRobotForm(!showRobotForm); }}>
            ...
            {showRobotForm && (
              <div className="form-card" style={{marginBottom: '2rem'}}>
                ...
                <div className="form-group">
                  ...
                  <input type="text" value={robotForm.nombre}
                    onChange={(e) => setRobotForm({...robotForm, nombre: e.target.value})} required />
                </div>
                <div className="form-group">
                  <label>Descripción *</label>
                  <textarea value={robotForm.descripcion}
                    onChange={(e) => setRobotForm({...robotForm, descripcion: e.target.value})} rows="3" required />
                </div>
                <div style={{display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem'}}>
                  <div className="form-group">
                    <label>Categoría *</label>
                    <select value={robotForm.categoriaId}
                      onChange={(e) => setRobotForm({...robotForm, categoriaId: e.target.value})} required>
                      <option value="">Selecciona categoría</option>
                      {categorias.map(cat => (
                        <option key={cat.id} value={cat.id}>{cat.nombre} (máx: {cat.pesoMaximo}g)</option>
                      ))}
                    </select>
                  </div>
                  <div className="form-group">
                    <label>Peso (gramos) *</label>
                    <input type="number" value={robotForm.peso}
                      onChange={(e) => setRobotForm({...robotForm, peso: e.target.value})} required min="1" />
                ...
                <div className="form-group">
                  ...
                  <textarea value={robotForm.especificacionesTecnicas}
                    onChange={(e) => setRobotForm({...robotForm, especificacionesTecnicas: e.target.value})}
                    rows="4" required placeholder="Describe los componentes, sensores, motores, etc." />
                </div>
                <div className="form-group">
                  <label>URL Foto del Robot (opcional)</label>
                  <input type="text" value={robotForm.fotoRobot}
                    onChange={(e) => setRobotForm({...robotForm, fotoRobot: e.target.value})}
                    placeholder="https://..." />
                </div>
                <button onClick={handleSubmitRobot} className="btn-primary" disabled={isAnyLoading}>
                  {isSavingRobot ? '⏳ Guardando...' : (editingRobot ? '💾 Actualizar' : '✅ Registrar')} Robot
                </button>
              </div>
            )}

            {loading ? <div className="loading">Cargando...</div> : misRobots.length === 0 ? (
              ...
              <div className="data-grid">
                {misRobots.map(robot => (
                  <div key={robot.id} className="card">
                    {robot.fotoRobot && (
                      <img src={robot.fotoRobot} alt={robot.nombre}
                    ...
                    <p><strong>Estado:</strong> <span className={`badge badge-${robot.estado.toLowerCase()}`}>{robot.estado}</span></p>
                    {robot.estado === 'RECHAZADO' && robot.motivoRechazo && (
                      <div className="alert alert-error" style={{marginTop: '1rem'}}>
                        <strong>❌ Motivo de rechazo:</strong>
                        <p style={{marginTop: '0.5rem', fontSize: '0.9rem'}}>{robot.motivoRechazo}</p>
                      </div>
                    )}
                    <div style={{display: 'flex', gap: '0.5rem', marginTop: '1rem'}}>
                      <button className="btn-secondary" style={{flex: 1}} disabled={isAnyLoading}
                        onClick={() => handleEditRobot(robot)}>✏️ Editar</button>
                      {robot.estado === 'PENDIENTE' && (
                        <button className="btn-secondary" style={{flex: 1, background: 'rgba(255,59,59,0.1)', borderColor: '#ff3b3b'}}
                          disabled={isAnyLoading} onClick={() => handleDeleteRobot(robot.id)}>
                          {isDeletingRobot ? '⏳...' : '🗑️ Eliminar'}
                        </button>
                      )}
                    </div>
                  </div>
        ...
        {activeTab === 'torneos' && (
          <div>
            ...
            {loading ? <div className="loading">Cargando...</div> : torneos.length === 0 ? (
                ...
                {torneos.map(torneo => (
                  <div key={torneo.id} className="card">
                    ...
                    <button className="btn-primary" style={{width: '100%', marginTop: '1rem'}}
                      disabled={isAnyLoading} onClick={() => handleAbrirInscripcion(torneo)}>
                      📝 Inscribirse
                    </button>
                  </div>
                ))}
        ...
        {activeTab === 'transferencias' && (
          <div>
            ...
            {solicitudesTransferencia.length === 0 ? (
              <p style={{textAlign: 'center', padding: '2rem', color: 'var(--muted)'}}>No tienes solicitudes de transferencia</p>
            ) : (
              <div className="data-grid">
                {solicitudesTransferencia.map(sol => (
                  <div key={sol.id} className="card">
                    <div style={{display: 'flex', justifyContent: 'space-between', alignItems: 'start'}}>
                      ...
                      <span className={`badge badge-${getBadgeClass(sol.estado)}`}>{getEstadoLabel(sol.estado)}</span>
                    </div>
                    {sol.mensajeUsuario && (
                    ...
                    <div style={{marginTop: '1rem', fontSize: '0.85rem'}}>
                      {sol.estado === 'PENDIENTE_SALIDA' && <p style={{color: '#ffa500'}}>⏳ Esperando aprobación de {sol.clubOrigenNombre}</p>}
                      {sol.estado === 'PENDIENTE_INGRESO' && <p style={{color: '#4CAF50'}}>✅ Salida aprobada. Esperando a {sol.clubDestinoNombre}</p>}
                      {sol.estado === 'APROBADA' && <p style={{color: '#00ff00'}}>🎉 ¡Transferencia completada!</p>}
                    ...
                    {(sol.estado === 'PENDIENTE_SALIDA' || sol.estado === 'PENDIENTE_INGRESO') && (
                      <button className="btn-secondary" disabled={isAnyLoading}
                        ...
                        {isCancelingTransfer ? '⏳ Cancelando...' : '❌ Cancelar Solicitud'}
                      </button>
                    )}
                  </div>
                ))}
          ...
          <div className="form-card" style={{maxWidth: '600px', width: '100%', maxHeight: '80vh', overflow: 'auto'}}>
            ...
            <div style={{display: 'flex', flexDirection: 'column', gap: '1rem'}}>
              {misRobots
                ...
                .map(robot => (
                  <div key={robot.id} className="card"
                    style={{cursor: isAnyLoading ? 'not-allowed' : 'pointer', border: '2px solid rgba(0,200,255,0.3)', opacity: isAnyLoading ? 0.6 : 1}}
                    onClick={() => !isAnyLoading && handleInscribirse(robot.id)}>
                    <div style={{display: 'flex', justifyContent: 'space-between', alignItems: 'center'}}>
                      ...
                      <button className="btn-primary" style={{pointerEvents: 'none'}} disabled={isAnyLoading}>
                        {isInscribiendo ? '⏳ Inscribiendo...' : 'Seleccionar →'}
                      </button>
                    </div>
                  </div>
                ))}
            </div>
            <button className="btn-secondary" style={{width: '100%', marginTop: '1.5rem'}} disabled={isAnyLoading}
              onClick={() => { setShowInscripcionModal(false); setSelectedTorneo(null); }}>
              ❌ Cancelar
            </button>
          </div>
        </div>
      ...
      {/* MODAL: Nueva solicitud de transferencia */}
      {showSolicitudModal && (
        <div style={{position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.9)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 9999}}>
          <div className="form-card" style={{maxWidth: '600px', width: '100%'}}>
            ...
            <div className="form-group">
              ...
              <select value={transferenciaForm.clubDestinoId}
            ...
            <div className="form-group">
              ...
              <textarea value={transferenciaForm.mensaje} rows="4" disabled={isAnyLoading}
            ...
            <div style={{display: 'flex', gap: '0.5rem'}}>
              ...
              <button className="btn-primary" disabled={isAnyLoading || !transferenciaForm.clubDestinoId}
                onClick={handleEnviarSolicitud}>
                {isEnviandoSolicitud ? '⏳ Enviando...' : '📨 Enviar Solicitud'}
              </button>
            </div>
          </div>
        </div>
